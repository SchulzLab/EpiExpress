...
for tissue in gtex_tissues:
    ...
    for eqtl in eqtl_types:
        ...
        eqtl_bed = pybedtools.BedTool('\n'.join(set([x.split('\t')[0] + '\t' + str(int(x.split('_')[1])-1) + '\t' + x.split('_')[1] for x in eqtl_list])), from_string=True)
    ...
    for score_col in ['score', 'score normG']:
        ...
        for eqtl in eqtl_types:
            ...
            re_res = gp.prerank(rnk=this_score_df, # CARE they sort themselves again by the first column.
                        gene_sets={model+'; '+score_col+'; '+sample_tissue_map[sample]: set(['\t'.join(x.fields[:3]) for x in inter_hits])},
                        min_size=1,
...
for model in models:
    ...
    model_recalls = process_pool.map(get_sample_recall, [[model, sample, False] for sample in list(sample_tissue_map.keys())])
    process_pool.close()
