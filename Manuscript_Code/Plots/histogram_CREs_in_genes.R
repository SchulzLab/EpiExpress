...
file_path <- "path/to/data/IHEC_Activity_1MB_hg38/"
...
kept_genes <- read.table("path/to/data/kept_genes.txt",
...
# Save results
write.table(col_vec_kept, "path/to/data/number_of_CREs_kept_genes.txt",
            col.names = FALSE, quote = FALSE)
write.table(col_vec, "path/to/data/number_of_CREs_All_genes.txt",
            col.names = FALSE, quote = FALSE)

# Generate histogram only if col_vec is non-empty
if (length(col_vec) > 0) {
  pdf(file = "path/to/data/hist_CREs_kep_genes.pdf", width = 5, height = 5)
