      ...
      for (k in 1:length(learning_rates)) {
        ...
        for (m in 1: length(batchsizes)) {
          ...
          for (b in 1: length(layers)) {
            ...
            if(layers[b] == 1){
              print("enter to the last loop first hidden layer")
              ...
              print("enter to the last loop second hidden layer")
              # ####b) Defining the DNN model
              # build_model<-function() {
              model <- keras_model_sequential()
              model %>%
                ...
                layer_dropout(rate = dropouts[j]) %>%
                layer_dense(nodes2[i], activation = "relu")   %>%
                
                layer_dropout(rate = dropouts2[j]) %>%
